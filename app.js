...
btnOpenCam?.addEventListener('click', async () => {
  try {
    ...
    statusEl.textContent = 'Error al abrir cámara';
  }
});
...
btnShot?.addEventListener('click', async () => {
  ...
  lastCapture = {
    ...
    blob,
    meta: { address: `${addr1} — ${addr2}`, coords: currentCoords, mapsUrl, deviceTs: new Date().toISOString(), brand: BRAND }
  };
...
let uid = null;
signInAnonymously(auth).catch(() => statusEl.textContent = 'Auth anónima falló');
...
async function uploadEvidence({ id, blob, meta }) {
  ...
  await addDoc(collection(db, 'evidences'), { owner: uid, path, ...meta, serverTs: serverTimestamp(), clientAgent: navigator.userAgent });
}
btnUpload?.addEventListener('click', async () => {
  ...
  try {
    ...
    else { await addToQueue(item); statusEl.textContent = 'Sin conexión: en cola'; }
    lastCapture = null;
  } catch (e) {
    await addToQueue(item); statusEl.textContent = 'Error subiendo: guardado en cola';
  } finally { updateQueueCount(); }
});
btnRetry?.addEventListener('click', async () => {
  const items = await getAllQueue();
  for (const it of items) { try { await uploadEvidence(it); await clearItem(it.id); } catch {} }
  statusEl.textContent = 'Reintentos finalizados';
  updateQueueCount();
});
